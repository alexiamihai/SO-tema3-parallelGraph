...
os_task_t *dequeue_task(os_threadpool_t *tp)
{
	...
	tp->sleeping_threads++;
	while (queue_is_empty(tp) && tp->start == 0)
		pthread_cond_wait(&tp->work, &tp->mutex);

	if (!tp->is_shutdown && !list_empty(&tp->head)) {
		t = list_entry(tp->head.prev, os_task_t, list);
		list_del(tp->head.prev);
		tp->sleeping_threads--;
	}
	if (tp->sleeping_threads == tp->num_threads) {
		tp->is_shutdown = 1;
...
os_threadpool_t *create_threadpool(unsigned int num_threads)
{
	...
	pthread_cond_init(&tp->work, NULL);
	tp->sleeping_threads = 0;
	tp->is_shutdown = 0;
	tp->start = 0;
